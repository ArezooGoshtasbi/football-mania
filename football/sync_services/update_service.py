...
class UpdateService:
    ...
    def sync_to_latest(self):
        ...
        if len(matches) > 0:
            ...
            self.sync_service.update_standings(standings)
            ...
            api_matches = self.api_client.fetch_matches(dateFrom=matches[0].utc_date.strftime("%Y-%m-%d"), dateTo=date_to.strftime("%Y-%m-%d"))
            for match in matches:
                for api_match in api_matches:
                    if match.id == api_match["id"]:
                        # Save match in DB
                        self.sync_service.save_match(api_match)
                        # Update predictions
